...
//! for enterprise Gemini API usage.
...
impl QuotaManager
{
  ...
  #[ inline ]
  pub fn check_quota( &self, request : &RequestMetadata ) -> Result< QuotaStatus, Error >
  {
  ...
  /// Panics if the usage tracker mutex is poisoned.
  #[ inline ]
  pub fn reserve_quota( &self, request : &RequestMetadata ) -> Result< QuotaReservation, Error >
  {
  ...
  /// Panics if the usage tracker mutex is poisoned.
  #[ inline ]
  pub fn get_quota_usage( &self ) -> Result< QuotaUsage, Error >
  {
    let mut tracker = self.usage_tracker.lock().unwrap();
    Self::reset_counters_if_needed( &mut tracker );
  ...
  fn build_per_user_usage( tracker : &QuotaUsageTracker ) -> HashMap< String, UserQuotaUsage >
  {
    ...
    users_by_usage.sort_by_key( | b | std::cmp::Reverse( b.1.daily_requests ) );
