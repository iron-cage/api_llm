...
Fix six violation categories discovered during the 2026-06-13 audit: missing `#[cfg(feature = "integration")]` guards on test files where no module-level gate exists, silent-pass/skip patterns (~34 functions returning instead of panicking), duplicated integration test helper code (get_api_key_* ×12), wrong model identifier in two integration test calls in `inference_tests.rs`, `async fn main()` in `providers_api_integration_test.rs`, and inline `#[cfg(test)] mod tests { }` block in `src/reliability/circuit_breaker.rs`. Observable outcome: `w3 .test l::3` → 0 failures, 0 warnings; all integration tests are gated; no integration helper duplication; no src-level inline test modules.
...
- `tests/providers_api_integration_test.rs` — convert `async fn main()` to a single `#[ cfg( feature = "integration" ) ] #[ tokio::test ] async fn test_providers_api_integration()`; use the shared `get_api_key_for_integration()` helper
...
| V06 | `async fn main()` instead of `#[tokio::test]` | 1 file | `tests/providers_api_integration_test.rs` |
...
7. **Convert fn main()** — in `tests/providers_api_integration_test.rs`, convert the entire `async fn main()` body into a single `#[ cfg( feature = "integration" ) ] #[ tokio::test ] async fn test_providers_api_integration()`. Use `get_api_key_for_integration()` for credentials; use `panic!` instead of `Box<dyn Error>` returns.
...
| T07 | No `async fn main` in tests/ | `tests/providers_api_integration_test.rs` | `grep -r "async fn main"` tests/ → 0 |
| T08 | No `#[cfg(test)] mod tests` in src/ | `src/reliability/circuit_breaker.rs` | `grep -r "#\[cfg(test)\]" src/` → 0 |
...
- `grep -r "async fn main"` across `tests/` → 0
- `grep -r "#\[cfg(test)\]" src/` → 0
...
- [ ] C8 — Does `tests/providers_api_integration_test.rs` contain zero instances of `async fn main`?
...
- [ ] AF2 — tests not deleted in providers_api_integration_test.rs: `grep -c "#\[tokio::test\]"` > 0
