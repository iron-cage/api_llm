...
Always pass relative paths (no leading slash) to `Url::join`. Use `"chat/completions"` not `"/v1/chat/completions"`, and `"models/{id}"` not `"/models/{id}"`. Ensure the base URL already contains the API version prefix with a trailing slash: `https://api-inference.huggingface.co/v1/`. Grep for `"/` (quote followed by slash) in any path strings passed to URL join or format calls to detect violations.

### Detection

Search for leading-slash string literals in endpoint path arguments:
```
grep -n '"/[a-z]' src/providers.rs src/inference.rs src/client.rs
```
Any match is a candidate violation. The grep pattern `"\/` in Rust string literals indicates an absolute path that will strip the base URL prefix.
